...
df <- data.frame(noms=names(intersect_negative_w_sort$x), valeurs=intersect_negative_w_sort$x)
...
ggplot(head(df, 15), aes(x = reorder(noms, -valeurs), y = valeurs)) +
...
# Tracer un graphique à barres horizontales
ggplot(head(df_positive, 15), aes(x = reorder(noms, -valeurs), y = valeurs)) +
  geom_bar(stat="identity") +
  labs(x="Noms", y="Valeurs")
